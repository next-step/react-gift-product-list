...
- **주문하기 기능**
  ...
  - [x] 주문하기 API에서 401 에러가 발생하면 로그인 페이지로
...
- **목표**
  - [x] Intersection Observer API를 사용하여 페이지네이션 구현하기
  - [x] axios외에 별도의 fetch 관련 라이브러리를 사용하지 않고 구현하기
  - [x] Suspense, ErrorBoundary도 사용하지 않기

<br />

- **테마 상품 목록 페이지**
  - [x] 선물하기 홈 > 선물 테마 섹션의 아이템을 클릭하면 테마 상품 목록 페이지로 연결되도록 하기
  - [x] /api/themes/:themeId/info API를 사용하여 선물 테마 섹션의 히어로 영역 구현하기
  - [x] 만약 테마 정보를 가져오는 API가 404 에러가 발생하면 선물하기 홈 페이지로 연결되도록 하기
  - [x] /api/themes/:themeId/products API를 사용하여 상품 리스트 구현하기
  - [x] 무한 스크롤 기능 구현하기
