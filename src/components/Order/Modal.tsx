...
function Modal({ modalToggle, fields, remove, append ,setModalToggle, price}: any) {
    ...
    const { watch, register, formState ,handleSubmit, setValue, } = useFormContext<OrderFormValues>();
                ...
                <ModalContent>
                    ...
                    {fields.map((field: any, index: any) => (
                        <ModalReceiverInputBox key={field.id}>
                            ...
                            {formState.errors.receivers?.[index]?.name && <ReceiverInputErrorTxt>이름을 입력해 주세요.</ReceiverInputErrorTxt>}
                            ...
                            {formState.errors.receivers?.[index]?.phone?.message && (<ReceiverInputErrorTxt>{formState.errors.receivers[index].phone?.message}</ReceiverInputErrorTxt>)}
                            ...
                            {formState.errors.receivers?.[index]?.count && <ReceiverInputErrorTxt>구매 수량은 1개 이상이어야 합니다.</ReceiverInputErrorTxt>}
